...
class Sequence : public std::series< Frame, Frame::TimeStamp > /* a.k.a. std::sorted_indexer< Frame::Stamped > */ {
 public:
    ...
    ///             Behaves like an *iterable* subcontainer-like iterator over input sequence (cf. std::range_iterator<>).
    ///
    ...
    ///
    Sequence operator[](const sensor::List& sensor_ids) const;
    ...
    /// @todo       Use a parameter pack instead of initializer list, discarding the use of {}.
    ///
    Sequence operator[](const sensor::Labels& sensor_labels) const;
    ...
    ///
    /// @return     Range instance encoding full sequence (thus, not incrementable).
    ///
    /// @note       Useful when passing full sequence to functions accepting a Range argument.
    ///
    operator Range();
    ...
    //--------------------------------------------------------------------------
    /// @brief      Builds a Signal instance holding channel data for given sensor *id* and given *channel*. Useful to encode
    ///             sensor/channel data to external non-trigno tools (e.g. plotting).
    ///
    /// @param[in]  range    Input range (data source).
    /// @param[in]  id       Sensor identifier.
    /// @param[out] values   Value container to store extracted values.
    /// @param[out] times    Timestamp container to store timestamps of extracted values. Defaults to nullptr (timestamps are discarded).
    ...
    ///
    static size_t extract(ConstRange range, sensor::ID id, std::vector< double >* values, std::vector< double >* times = nullptr, size_t channel = 0);
    ...
    /// @return     Instance of Signal with data for specified sensor/channel.
    ///
    static size_t extract(ConstRange range, const sensor::Label& sensor_label, std::vector< double >* values, std::vector< double >* times = nullptr, size_t channel = 0);
};
